...
# 골렘이 닿는 최하단 행을 저장하는 딕셔너리
exit_to_max_row = {}
visited_exit = set()  # 이미 계산된 출구 위치

# 출구의 최하단 행을 기록하는 함수
def get_max_row_for_exit(y, x):
    if (y, x) in exit_to_max_row:
        return exit_to_max_row[(y, x)]
    result = bfs(y, x)
    exit_to_max_row[(y, x)] = result
    return result
...
def down(y, x, d, id):
    if canGo(y + 1, x):
        ...
        if not inRange(y - 1, x - 1) or not inRange(y + 1, x + 1):
            ...
            global answer
            exit_pos = (y + dy[d], x + dx[d])
            if exit_pos not in visited_exit:
                answer += get_max_row_for_exit(y, x) - 3 + 1
                visited_exit.add(exit_pos)
